...
def get_unscheduled_tasks():
    ...
    for t in tasks:
        ...
        if not due_date_str or due_date_str < today.isoformat():
            unscheduled.append({
                "id": str(t["id"]),
...
tasks_today = [
    {"id": str(t["id"]), "content": t.get("content", ""), "due": t.get("due", {}).get("date")}  
    for t in resp.json()
    if t.get("due") and t["due"].get("date") and t["due"]["date"] <= today.isoformat()
]
