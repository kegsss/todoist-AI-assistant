...
def get_busy_slots():
    ...
    for t in tasks:
        ...
        dt = due_info.get("datetime")
        dur = t.get("duration") or cfg.get("default_task_duration_minutes", 60)
        if dt:
            dt = datetime.fromisoformat(dt.replace('Z', '+00:00')).astimezone(tz)
            d = dt.date()
            if d in busy:
                busy[d].append((dt, dt + timedelta(minutes=dur)))
    for d in busy:
...
if unscheduled:
    ...
    resp_msg = call_openai(messages, functions=[make_schedule_function()], function_call={"name": "assign_due_dates"})
