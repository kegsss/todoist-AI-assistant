...
BUFFER_MIN = cfg.get('buffer_minutes', 5)               # minutes between tasks
DECAY_PER_DAY = cfg.get('priority_decay_per_day', 1)     # priority levels per full day since creation
...
if tasks_today:
    ...
    for r in ranks:
        print(f"🏷 Setting priority {r['priority']} for task {r['id']}")
        requests.post(
